...
<head>
    ...
    <title>Kechki Zikrlar Ro'yxati</title>
    <style>
        body {
...
<div class="container">
    ...
    <c:if test="${user != null && user.role == 'admin'}">
        <a href="/views/add_kechki.html" class="add-zikr-btn">Yangi Zikr Qo'shish</a>
    </c:if>
    <!-- Zikrlar ro'yxati -->
    <c:forEach var="zikr" items="${zikrs}">
        <div class="zikr-item" onclick="toggleZikrContent(this)">
            ...
            <div class="zikr-content">
                ...
                <c:if test="${user != null && user.role == 'admin'}">
                    <div class="actions">
                        <a href="/kechki?action=edit&id=${zikr.id}">Yangilash</a>
                        <form action="/kechki?action=deleteZikr&id=${zikr.id}" method="post" style="display:inline;" onsubmit="return confirm('Haqiqatan o‘chirmoqchimisiz?');">
            ...
            <button class="save-btn"
                    ...
                    data-saved="${savedZikrs.contains(zikr.id) ? 'true' : 'false'}"
                    onclick="event.stopPropagation(); toggleSave(this);">
                    ${savedZikrs.contains(zikr.id) ? 'Saqlanganlardan ochirish' : 'Saqlanganlarga qoshish'}
            </button>
        </div>
    </c:forEach>
    <c:if test="${empty zikrs}">
